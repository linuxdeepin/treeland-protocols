...
#### `treeland-output-uuid-unstable-v1.xml` and `treeland-output-uuid-head-unstable-v1.xml`

New protocols. They let clients obtain a stable, opaque uuid string identifying the physical output device behind a `wl_output` or a `zwlr_output_head_v1`. Other treeland output protocols (output-manager-v2, output-mirror-manager-v1) now address outputs by these uuids instead of `wl_output` objects or output-name strings.

1. `treeland-output-uuid-unstable-v1.xml` defines the `treeland_output_uuid_v1` interface (a uuid identity object with a `uuid` event) and its first factory `treeland_output_uuid_manager_v1`, whose `get_uuid` request creates a uuid object from a `wl_output`.
2. `treeland-output-uuid-head-unstable-v1.xml` is a companion protocol depending on wlr-output-management. It defines only the second factory `treeland_output_uuid_head_manager_v1`, whose `get_uuid` request creates the same `treeland_output_uuid_v1` object from a `zwlr_output_head_v1`. It does not redefine `treeland_output_uuid_v1` and adds no requests or events to it.
3. `treeland_output_uuid_v1` is declared `frozen="true"` and stays at version 1 forever: it is created from two independent factory interfaces that descend from independent global ancestors (`treeland_output_uuid_manager_v1` from wayland core, `treeland_output_uuid_head_manager_v1` from wlr-output-management), so its version cannot advance with either parent. This mirrors the upstream `wl_buffer`/`wl_callback`/`ext_image_capture_source_v1` pattern.
4. The uuid is unique among currently connected outputs (the compositor must disambiguate, e.g. by connector when two devices share model+serial), stable across sessions and reconnections, and opaque to clients. It may change when a persistent device identity becomes available after an initial connector-based fallback, reported by a further `uuid` event.
5. A uuid object is not tied to the lifetime of the `wl_output`/head global: when the output is unplugged the object stays valid and the last uuid remains the device identity. Both factories return the same uuid for the same physical output.

Consumers should bind `treeland_output_uuid_manager_v1` (and `treeland_output_uuid_head_manager_v1` when they work with wlr heads), call `get_uuid`/`get_uuid_for_head`, and use the reported uuid string everywhere other treeland output protocols require output identity.

#### `treeland-output-manager-unstable-v2.xml`

Wire-incompatible change to the existing v2 protocol: output identity switched from `wl_output` objects to opaque uuid strings from `treeland-output-uuid-unstable-v1`. The interface and protocol names are unchanged (the experimental disclaimer permits backward-incompatible changes without a major-version bump). Wire-level differences:

1. `set_primary_output` argument changed from `object interface="wl_output"` to `string uuid`. An empty uuid is rejected with a new fatal manager `error.invalid_uuid`; a uuid that does not identify a connected enabled output is rejected with the existing non-fatal `primary_output_failed` event (reason `invalid_output`).
2. The `primary_output` event argument changed from `object interface="wl_output" allow-null="true"` to `string uuid`; an empty uuid means no primary output is currently designated (Wayland strings have no null, so the empty string replaces the former null object).
3. `get_picture_control` argument changed from `object interface="wl_output"` to `string uuid`. An empty uuid is rejected with `error.invalid_uuid`; any non-empty uuid is accepted and validity is reported at commit time through the existing `commit_result.invalid_output`, matching the prior "accept any live output, fail at commit" philosophy.
4. `primary_output_failed_reason.invalid_output` and `commit_result.invalid_output` summaries now read "no enabled output with the provided uuid" / "the uuid does not identify a live enabled output"; their values are unchanged.
5. The `allow-null="true"` attribute is removed from the protocol (no nullable object args remain). Interface versions stay at 1.

Consumers should obtain output uuids from `treeland_output_uuid_manager_v1.get_uuid`, pass uuid strings to `set_primary_output`/`get_picture_control`, and treat an empty uuid in `primary_output` as "no primary output".
...
5. The per-group `error` event is removed; validation failures are now fatal protocol errors posted with specific enum entries. Manager errors: `invalid_name` (empty, non-UTF-8, or NUL-containing name) and `name_exists` (duplicate). Group errors: `invalid_output` (no enabled output with the provided uuid), `duplicate_output` (already a member of this group), `output_in_use` (member of another group), `not_in_group` (remove of a non-member), and `already_dissolved` (request after the group was dissolved). The old codes `invalid_group_name` (0), `invalid_screen_number` (1), and `invalid_output` (2) no longer exist.
